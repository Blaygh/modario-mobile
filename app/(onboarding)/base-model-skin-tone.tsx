...
export default function BaseModelSkinToneScreen() {
  ...
  useEffect(() => {
    const restoredPresetId = restoredDraft?.skinTonePresetId ?? draft?.avatarSkinTonePresetId ?? previewOptions[0]?.id ?? null;
    if (restoredPresetId) {
      setSelectedPresetId(restoredPresetId);
    }
  }, [draft?.avatarSkinTonePresetId, previewOptions, restoredDraft?.skinTonePresetId]);
  ...
  const handleContinue = async () => {
    ...
    await saveDraft(
      ...
      { screen: 'base-model-skin-tone', step: 'avatar_skin_tone' },
    );
