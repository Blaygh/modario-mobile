...
export default function BaseModelConfirmScreen() {
  ...
  const { draft, saveDraft, saveError } = useOnboardingSession();
  ...
  const bodyTypePresetId = restoredDraft?.bodyTypePresetId ?? draft?.avatarBodyTypePresetId ?? null;
  ...
  useEffect(() => {
    const preferred = draft?.avatarBaseModelId ?? currentAvatarQuery.data?.baseModelId ?? matchingModels[0]?.id ?? null;
    if (preferred) {
      setSelectedModelId(preferred);
    }
  }, [currentAvatarQuery.data?.baseModelId, matchingModels, draft?.avatarBaseModelId]);
  ...
  const handleConfirm = async () => {
    ...
    await saveDraft(
      ...
      { screen: 'base-model-confirm', step: 'avatar_base_model' },
    );
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.ivory }}>
      <View className="flex-1 px-4 py-4">
          ...
          {matchingModels.map((model) => {
            ...
            return (
              <Pressable
                ...
                <Image source={{ uri: model.imageUrl ?? fallbackBaseModel }} style={{ width: '100%', height: 260 }} contentFit="cover" cachePolicy="memory-disk" />
        ...
        {selectMutation.isError || Boolean(saveError) ? (
          <Text className="mt-4 font-InterRegular text-sm leading-6" style={{ color: '#B42318' }}>
            {(selectMutation.error instanceof Error && selectMutation.error.message) || (saveError instanceof Error && saveError.message) || 'We could not save this base model. Please retry.'}
          </Text>
        ) : null}
        ...
        <View className="pb-2 pt-4" style={{ gap: 12 }}>
          ...
          <View style={{ gap: 10 }}>
            <SecondaryButton label="Back" onPress={() => router.back()} disabled={selectMutation.isPending} />
            <PrimaryButton label="Confirm and continue" fullWidth onPress={handleConfirm} disabled={!selectedModelId || selectMutation.isPending} loading={selectMutation.isPending} />
          </View>
        </View>
      </View>
    </SafeAreaView>
