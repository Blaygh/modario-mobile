...
export default function ImportReviewScreen() {
  ...
  useEffect(() => {
    const loadDetails = async () => {
      ...
      try {
        setLoading(true);
        const detail = await getWardrobeImportDetails(
          session.access_token,
          sessionId,
        );
        setSourceImageUrl(detail.source_image?.storage_url ?? null);
        setDecisions(
          detail.detected_items.map((item) => ({ item, include: true })),
        );
      } catch (loadError) {
        setError(
  ...
  const selectedCount = useMemo(
    () => decisions.filter((decision) => decision.include).length,
    [decisions],
  );
  ...
  const onCommit = async () => {
    ...
    try {
      ...
      const nextCount = countSoFar + selectedCount;
  ...
  return (
    ...
    >
      ...
      <Text
        className="mb-3 font-InterRegular text-sm leading-6"
        style={{ color: palette.muted }}
      >
        Confirm each detected item before adding it to your wardrobe.
      </Text>
      {sessionId && sessionIds.length > 1 ? (
        <Text
          className="mb-3 font-InterRegular text-sm"
          style={{ color: palette.muted }}
        >
          Reviewing photo {currentIndex + 1} of {sessionIds.length}.
        </Text>
      ) : null}
      {loading ? (
        <Text
      ...
      {!loading && !decisions.length ? (
        <EmptyState
          ...
          description="This import session does not have reviewable items yet."
        />
      ) : null}
      ...
      >
        <View className="gap-3">
          {decisions.map((decision) => (
            <View
              key={decision.item.detected_item_id}
            ...
            >
              <View className="flex-row gap-3">
                <Image
                  source={{
                    uri:
                      decision.item.crop_storage_url ??
                      sourceImageUrl ??
                      fallbackImage,
                  }}
                ...
                <View className="flex-1 justify-between">
                  <View>
                    ...
                    >
                      {decision.item.label ?? "Detected item"}
                    </Text>
                    <Text
                      className="mt-1 font-InterRegular text-sm"
                      style={{ color: palette.muted }}
                    >
                      {decision.item.role_suggestion ?? "unknown role"} •{" "}
                      {Math.round((decision.item.confidence ?? 0) * 100)}%
                      confidence
                    </Text>
                    <Text
                      className="mt-2 font-InterRegular text-xs leading-5"
                      style={{ color: palette.muted }}
                    >
                      {Object.entries(decision.item.attributes_preview ?? {})
                        .slice(0, 3)
                        .map(([key, value]) => `${key}: ${String(value)}`)
                        .join(" • ") || "No preview attributes"}
                    </Text>
                  </View>
                    ...
                    <Switch
                      ...
                      onValueChange={(include) =>
                        setDecisions((current) =>
                          current.map((entry) =>
                            entry.item.detected_item_id ===
                            decision.item.detected_item_id
                              ? { ...entry, include }
                              : entry,
                          ),
                        )
      ...
      <View className="gap-3 pb-2 pt-4">
        ...
        <SecondaryButton
          ...
          onPress={() =>
            setDecisions((current) =>
              current.map((decision) => ({ ...decision, include: true })),
            )
          }
        />
      </View>
