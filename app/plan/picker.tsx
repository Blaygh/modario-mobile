...
export default function PlanOutfitPickerScreen() {
  ...
  const [selectedOutfitId, setSelectedOutfitId] = useState<string | null>(
    params.outfitId ?? null,
  );
  const [selectedDate, setSelectedDate] = useState(
    params.plannedDate ?? dates[0],
  );
  ...
  const onSubmit = async () => {
    try {
      ...
      if (
        !stableOutfitId &&
        params.sourceMode === "candidate" &&
        params.candidateId
      ) {
        const saved = await saveCandidateMutation.mutateAsync({
      ...
      if (!stableOutfitId) {
        Alert.alert(
          "Choose an outfit",
          "Select a saved outfit before planning it.",
        );
        return;
      }
      ...
      router.replace({
        ...
        params: { date: selectedDate, planId: planned.id },
      });
    } catch (error) {
      Alert.alert(
        "Unable to plan outfit",
  ...
  const plannedFromCandidate =
    params.sourceMode === "candidate" && params.candidateId;
  ...
  return (
    ...
    >
      ...
      >
        <Text
          className="font-InterRegular text-sm leading-6"
        ...
        <Text
          className="mt-5 font-InterSemiBold text-base"
        ...
        {savedQuery.isLoading ? (
          <Text
            className="mt-3 font-InterRegular text-sm"
            style={{ color: palette.muted }}
          >
        ...
        !plannedFromCandidate ? (
          <EmptyState
            ...
            description="Save a recommendation first, then come back to plan it."
          />
        ) : null}
