...
export default function PlannerCalendarScreen() {
  ...
  const [selectedDate, setSelectedDate] = useState(dayOptions[0]?.value ?? range.from);

  const plannedForSelectedDate = (plannedQuery.data ?? []).filter((entry) => entry.plannedDate === selectedDate);
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View className="flex-row gap-2 pb-2">
          {dayOptions.map((option) => (
            <FilterChip key={option.value} label={option.label} selected={option.value === selectedDate} onPress={() => setSelectedDate(option.value)} />
          ))}
        </View>
      </ScrollView>
      <SectionHeader title="Planned outfits" action={prettyDate(selectedDate)} />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
        {plannedQuery.isLoading ? <InfoNotice title="Loading planner" description="We’re fetching your live plan for the coming weeks." /> : null}
        {plannedQuery.isError ? (
          <EmptyState title="Planner unavailable" description="We couldn’t load your planned outfits right now." action={<PrimaryButton label="Retry" onPress={() => plannedQuery.refetch()} />} />
        ) : null}
        {!plannedQuery.isLoading && !plannedQuery.isError && !(plannedQuery.data?.length ?? 0) ? (
          <EmptyState title="No planned outfits" description="Save an outfit, then plan it for a date that matters." />
        ) : null}
        {!plannedQuery.isLoading && !plannedQuery.isError && (plannedQuery.data?.length ?? 0) > 0 && !plannedForSelectedDate.length ? (
          <EmptyState title="Nothing planned that day" description="Pick another date above or plan a saved outfit to fill this day." action={<PrimaryButton label="Plan an outfit" onPress={() => router.push('/plan/picker')} />} />
        ) : null}
        <View className="gap-3">
          {plannedForSelectedDate.map((entry) => (
            <Pressable
              ...
              <Text className="mt-3 font-InterMedium text-sm" style={{ color: palette.burgundy }}>Open day detail</Text>
            </Pressable>
          ))}
        </View>
      </ScrollView>
