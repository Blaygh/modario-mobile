          ...
          <View className="gap-4">
            {recommendationsQuery.isLoading ? (
              <InfoNotice
                ...
                description="We’re fetching your latest recommendation feed."
              />
            ) : null}
            {recommendationsQuery.isError ? (
              <EmptyState
                title="Suggestions unavailable"
                description="Please retry to load live recommendations."
                action={
                  <PrimaryButton
                    label="Retry"
            ...
            !(recommendationsQuery.data?.length ?? 0) ? (
              <EmptyState
                ...
                description="Add more wardrobe pieces to generate recommendation candidates."
              />
            ) : null}
            {(recommendationsQuery.data ?? []).map((outfit) => (
              ...
              >
                ...
                <Text
                  className="mt-3 font-InterSemiBold text-xl"
                  style={{ color: palette.ink }}
                >
                  {outfit.summary}
                </Text>
                <Text
                  className="mt-2 font-InterRegular text-sm leading-6"
                  style={{ color: palette.muted }}
                >
                  {outfit.roles.length
                    ? `${outfit.roles.map((role) => role.role).join(" • ")}.`
                    : "Open detail to review the pieces and style notes."}
                </Text>
                <Pressable
                  onPress={() =>
                    router.push({
                  ...
                  className="mt-4 self-start rounded-full bg-[#F3E8EC] px-4 py-2"
                >
                  <Text
        ...
        {active === "Saved" ? (
          <View className="gap-4">
            {savedQuery.isLoading ? (
              <InfoNotice
                title="Loading saved looks"
                description="We’re syncing the outfits you’ve saved."
              />
            ) : null}
            {savedQuery.isError ? (
              <EmptyState
                title="Saved outfits unavailable"
                description="Please retry to load your outfit library."
                action={
                  <PrimaryButton
                    label="Retry"
            ...
            !(savedQuery.data?.length ?? 0) ? (
              <EmptyState
                ...
                description="Save a recommendation to keep it in your library and planner."
              />
            ) : null}
            {(savedQuery.data ?? []).map((outfit) => (
              <Pressable
                ...
                onPress={() =>
                  router.push({
                    pathname: "/outfit/[id]",
                    params: { id: outfit.id, mode: "saved" },
                  })
                }
                className="rounded-[22px] border bg-white p-4"
                style={{ borderColor: palette.line }}
              >
                ...
                <Text
                  className="mt-2 font-InterRegular text-sm"
                  style={{ color: palette.muted }}
                >
                  Saved on {formatDate(outfit.createdAt) || "recently"}.
                </Text>
                  ...
                  <Pressable
                    onPress={() =>
                      router.push({
                        ...
                        params: { outfitId: outfit.id, sourceMode: "saved" },
                      })
                    }
                    className="self-start rounded-full border bg-white px-4 py-3"
                    style={{ borderColor: palette.line }}
                  >
                    <Text
                      className="font-InterMedium text-sm"
          ...
          <View className="gap-4">
            {plannedQuery.isLoading ? (
              <InfoNotice
                ...
                description="We’re pulling your live planned outfits."
              />
            ) : null}
            {plannedQuery.isError ? (
              <EmptyState
                title="Planner unavailable"
                description="Please retry to load your upcoming looks."
                action={
                  <PrimaryButton
                    label="Retry"
            ...
            !(plannedQuery.data?.length ?? 0) ? (
              <EmptyState
                ...
                description="Plan a saved outfit to see it here and in your calendar."
              />
            ) : null}
