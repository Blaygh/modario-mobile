...
export default function HomeScreen() {
  ...
  const onSaveRecommendation = async (candidateId: string) => {
    try {
      const savedOutfit = await saveCandidateMutation.mutateAsync({
        candidateId,
      });
    ...
    >
      ...
      >
        ...
        {recommendationsQuery.isLoading ? (
          <InfoNotice
            title="Styling your recommendations"
            description="We’re gathering live outfit suggestions from your wardrobe."
          />
        ) : null}
        ...
        {plannedTodayQuery.isLoading ? (
          <InfoNotice
            title="Checking today's plan"
            description="We're loading any outfits already planned for today."
          />
        ) : null}
            ...
            <SectionHeader
              title={
                plannedToday.length === 1
                  ? "Planned for today"
                  : "Today's planned outfits"
              }
              action={`${plannedToday.length} ${plannedToday.length === 1 ? "look" : "looks"}`}
            />
                ...
                >
                  ...
                  <View className="flex-1">
                    ...
                    >
                      {plannedToday.length === 1
                        ? "Ready for today"
                        : `Slot ${entry.slotIndex + 1} for today`}
                    </Text>
                    {entry.notes ? (
                      <Text
                        className="mt-1 font-InterRegular text-sm leading-5"
                        numberOfLines={2}
                        style={{ color: palette.muted }}
                      >
                        {entry.notes}
        ...
        {hero ? (
          <Pressable
            onPress={() =>
              router.push({
                pathname: "/outfit/[id]",
                params: { id: hero.id, mode: "candidate" },
              })
            }
          >
            <View
            ...
            >
              <Image
                source={{ uri: hero.previewImageUrl ?? fallbackLook }}
                style={{ width: "100%", height: 208 }}
                contentFit="cover"
              />
              <LinearGradient colors={[palette.burgundy, palette.burgundySoft]}>
                <View className="p-5">
                  <Text className="font-InterBold text-[28px] leading-[32px] text-white">
                    Today&apos;s Outfit
                  </Text>
                  <Text className="mt-2 font-InterRegular text-sm leading-6 text-[#F8EAF0]">
                    {hero.summary}
                  </Text>
                  ...
                  <View className="mt-4 flex-row flex-wrap gap-2">
                    ...
                    <SecondaryButtonCompact
                      label="Save"
                      onPress={() => onSaveRecommendation(hero.candidateId)}
                    />
                    <SecondaryButtonCompact
                      label="Plan"
                      onPress={() =>
              ...
              {secondary.map((outfit) => (
                <Pressable
                  ...
                  onPress={() =>
                    router.push({
                      pathname: "/outfit/[id]",
                      params: { id: outfit.id, mode: "candidate" },
                    })
                  }
                ...
                >
                  <Image
                    source={{ uri: outfit.previewImageUrl ?? fallbackLook }}
                  ...
                  <View className="p-3">
                    ...
                    <Text
                      className="mt-3 font-InterSemiBold text-base"
                      numberOfLines={2}
                      style={{ color: palette.ink }}
                    >
                      {outfit.summary}
                    ...
                    <View className="mt-3 flex-row items-center gap-2">
                      <SecondaryButtonCompact
                        label="Save"
                        onPress={() => onSaveRecommendation(outfit.candidateId)}
                      />
                      <Pressable
                        onPress={() =>
                          router.push({
        ...
        >
          <Text
            ...
            style={{ color: palette.burgundySoft }}
          >
            Live wardrobe overview
          </Text>
          <Text
            className="mt-2 font-InterSemiBold text-xl"
            style={{ color: palette.ink }}
          >
            {wardrobeQuery.data?.length
              ? `${wardrobeQuery.data.length} ready-to-style pieces in your wardrobe.`
              : "Your wardrobe is ready for its next edit."}
          </Text>
          <Text
            className="mt-1 font-InterRegular text-sm leading-6"
            style={{ color: palette.muted }}
          >
            {wardrobeQuery.data?.length
...
function SecondaryButtonCompact({
  label,
  onPress,
}: {
  label: string;
